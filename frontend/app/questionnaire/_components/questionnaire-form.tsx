...
const weddingInitial = Object.fromEntries(
  WEDDING_FIELDS.map((field) => [field.key, ""])
);

const jubileeInitial = Object.fromEntries(
  JUBILEE_FIELDS.map((field) => [field.key, ""])
);
  ...
  return (
    <div className="space-y-4">
      <section className="grid gap-3 sm:grid-cols-2">
        <Field label="Имя клиента / название заявки *"><input value={formData.clientName} onChange={onChange("clientName")} className={inputClass} /></Field>
        <Field label="Телефон *"><input value={formData.phone} onChange={onChange("phone")} className={inputClass} /></Field>
        <Field label="Дата события *"><input type="date" value={formData.eventDate} onChange={onChange("eventDate")} className={inputClass} /></Field>
        <Field label="Город *"><input value={formData.city} onChange={onChange("city")} className={inputClass} /></Field>
        <Field label="Площадка *"><input value={formData.venue} onChange={onChange("venue")} className={inputClass} /></Field>
        <Field label="Старт / сбор гостей"><input value={formData.startTime} onChange={onChange("startTime")} className={inputClass} /></Field>
        <Field label="Количество гостей"><input value={formData.guestCount} onChange={onChange("guestCount")} className={inputClass} /></Field>
        <Field label="Дети на событии"><input value={formData.childrenInfo} onChange={onChange("childrenInfo")} className={inputClass} /></Field>
      </section>
